...
describe('switchPath basic usage', () => {
    ...
    it.skip('should match a path on an incomplete pattern', () => {
        const {path, value} = switchPath('/home/foo', {
            '/bar': 123,
            '/home': 456,
        });
    ...
    it.skip('should match an incomplete pattern on multipart path', () => {
        const {path, value} = switchPath('/home/foo/bar', {
            '/home': {
                '/': 123,
                '/foo': 456
    ...
    it.skip('should not prematurely match a notFound pattern', () => {
        const {path, value} = switchPath('/home/foo', {
            '*': 0,
            '/bar': 123,
            '/home': 456,
...
describe('switchPath corner cases', () => {
    ...
    it.skip('should partially match :key type params', () => {
    ...
    it.skip('should partially match multiple :key type params', () => {
...
describe(`switch-path-ruta3 cases which don't work with switch-path 1.18`, () => {
    it('should match url with one placeholder even if a url with two place holder is defined', () => {
        const {path, value} = switchPath('/bar/123', {
            '/foo': 123,
            '/bar/:issueId': issueId => `issue ${issueId}`,
            '/bar/:issueId/:articleId': (issueId, articleId) => `issue ${issueId} article ${articleId}`,
        });

        expect(path).to.be.equal('/bar/123')
        expect(value).to.be.equal('issue 123') // switch-path returns 'issue 123 article undefined'
    })

    it('should solve issue staltz/switch-path#23', () => {
        ...
        expect(path).to.be.equal('/1736/home') // switch-path returns '/1736'
        expect(value).to.be.equal(789) // switch-path returns 'id is 1736'
    })
});
